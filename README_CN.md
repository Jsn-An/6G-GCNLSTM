...
- [1. 项目简介](#1-项目简介)
- [2. 快速导航（给不同读者）](#2-快速导航给不同读者)
- [3. 项目结构与文件索引](#3-项目结构与文件索引)
  - [3.1 数据集](#31-数据集)
  - [3.2 数据处理与图构建](#32-数据处理与图构建)
  - [3.3 预测模型（GCN+LSTM）](#33-预测模型gcnlstm)
  - [3.4 模型评估](#34-模型评估)
  - [3.5 任务卸载优化](#35-任务卸载优化)
  - [3.6 智能调度](#36-智能调度)
  - [3.7 可视化结果](#37-可视化结果)
- [4. 完整实验流程](#4-完整实验流程)
- [5. 结果汇总](#5-结果汇总)
...
| 你想知道什么 | 去哪里看 |
|---|---|
| 项目整体是干嘛的 | [1. 项目简介](#1-项目简介) |
| 实验流程怎么走的 | [4. 完整实验流程](#4-完整实验流程) |
| 最终效果怎么样 | [5. 结果汇总](#5-结果汇总) |
| 生成的可视化图表 | [3.7 可视化结果](#37-可视化结果) |
...
| 你想了解什么 | 去哪里看 |
|---|---|
| 怎么安装运行 | [6. 环境配置与运行指南](#6-环境配置与运行指南) |
| 数据是怎么处理的 | [3.2 数据处理与图构建](#32-数据处理与图构建) |
| 模型架构长什么样 | [3.3 预测模型（GCN+LSTM）](#33-预测模型gcnlstm) 和 [7. 技术方法详解](#7-技术方法详解) |
| 所有代码文件在哪 | [3. 项目结构与文件索引](#3-项目结构与文件索引) |
| 卸载策略怎么实现的 | [3.5 任务卸载优化](#35-任务卸载优化) |

### 📊 如果你想看实验结果

| 你想看什么 | 去哪里看 |
|---|---|
| 预测精度指标 | `results/metrics/per_node_metrics.csv` |
| 卸载策略对比 | `results/metrics/offloading_strategy_comparison.csv` |
| 调度策略对比 | `results/metrics/scheduling_policy_comparison.csv` |
| 所有可视化图 | `reports/figures/` 目录 |
...
### 5.1 预测性能（GCN+LSTM vs Baselines）
